...
int IGameController::OnCharacterDeath(class CCharacter *pVictim, class CPlayer *pKiller, int Weapon)
{
	...
	if(g_Config.m_SvKillingspree && g_Config.m_SvKillingspree_kills <= pVictim->GetPlayer()->killstreak)
	{
		...
		str_format(buf, sizeof(buf), "%s's killing spree\n(%d kills)\nhas ended by %s", Server()->ClientName(pVictim->GetPlayer()->GetCID()), pVictim->GetPlayer()->killstreak, Server()->ClientName(pKiller->GetCID()));
...
void IGameController::make_instagib(char *new_gametype)
{
	instagib = true;
	m_pGameType = new_gametype;
}

void IGameController::make_vanilla(char *new_gametype)
{
	instagib = false;
	m_pGameType = new_gametype;
}

bool IGameController::is_instagib() const
{
	return instagib;
}
